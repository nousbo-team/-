...
| 1차 검토·승인 · **최종파일 관리 창구** · **반려 판단권(공동)** | 박현경 팀장 (부재 시 김신덕 본부장) | 브랜드기획팀 | 파일 최종본 여부 판단, 수정요청 또는 연구소 이관 결정. **최종 승인파일의 관리 및 울산공장 전달을 담당하는 단일 접점(Single Point of Contact)**. 최종검수 대기 중에도 **문제를 발견하면 직접 반려 가능**(반려 시 사유 필수) |
| 디자인 수정 | 최효진 차장 | 디자인 | 지적사항에 따른 디자인파일 수정 |
| 최종 검수·승인 · **반려 판단권(공동)** | 조현종 소장 | 연구소 | 표시사항/법적요건 등 최종 검수 및 승인. **반려 판단은 연구소와 브랜드기획팀(박현경 팀장/김신덕 본부장)이 공동으로 수행**(반려 시 사유 필수 기재) — 단, 승인·수정필요(경미) 판정은 연구소 전용 |
...
| 4 | 조현종 소장 → 박현경 팀장 | 최종 검수 후 결과 통보. 결과는 3가지 — **승인** / **수정 필요**(경미한 보완, 2단계로 회귀, 연구소 전용) / **반려**(부적합 판정, **반려사유를 반드시 기재**하여 2단계로 회귀). **반려는 연구소(조현종 소장) 또는 브랜드기획팀(박현경 팀장/김신덕 본부장)이 각자 판단해 처리 가능** — 최종검수 대기 중 브랜드기획팀이 문제를 먼저 발견해도 협의 없이 바로 반려 처리 가능 | 이메일 |
...
- **반려 처리 권한을 연구소(조현종 소장) + 브랜드기획팀(박현경 팀장/김신덕 본부장)으로 제한** — 승인·수정필요(경미)는 연구소 전용, 반려만 두 역할 공동 수행 가능하도록 권한 분리 *(2026-07-28 업데이트: 기존 연구소 단독 → 브랜드기획팀 공동 판단으로 변경)*
- **각 담당자별 처리 화면에서 바로 반려/취소 가능** — 요청자는 본인 요청을 완료 전 언제든 취소, 1차검토 단계 반려는 요청 취소로, 승인 후 반려는 최종검수로, 디자인 단계 반려는 1차검토로 되돌아가며, 처리 시 사유 입력이 필수이고 그 건을 거쳐간 이전 담당자 전원에게 알림이 간다 *(2026-07-29 추가)*
