...
1. `haon`으로 로그인 → 대시보드 → "발주요청 등록" → 품목([품목코드] 품목명) 선택 후 등록 → 자동 채번된 요청번호(`RQ-YYYYMMDD-###`)가 부여됩니다.
...
반려/취소: 대시보드의 "내가 처리해야 할 건" 목록에서 각 건마다 반려/취소 버튼으로 바로 처리할 수 있습니다(상세 페이지를 열지 않아도 됨). 클릭하면 사유 입력창이 뜨고, 사유를 입력해야만 처리됩니다.
- `haon`(요청자)은 본인이 등록한 건을 완료 전까지 언제든 "요청 취소"할 수 있습니다.
- `isis9`(1차검토)은 1차검토중 건을 반려하면 요청이 취소되고, 승인(전달 대기) 건을 반려하면 최종검수중으로 되돌립니다.
- `guychj`(디자인)은 배정된 수정 작업을 반려하면 1차검토중으로 되돌립니다.
- 반려/취소되면 처리한 본인을 포함해 그 건을 거쳐간 이전 담당자 전원에게 알림이 갑니다.
...
권한 분리(승인·수정필요는 연구소만, 반려는 연구소+1차검토 공동), 반려/취소 사유 필수, 단계별 반려·되돌리기(요청 취소, 디자인→1차검토, 승인→최종검수), 중복 요청 감지, 3개월 예외, 대체 담당자 라우팅, 파일 잠금/버전 승계 등 핵심 비즈니스 로직을 자동 테스트로 검증합니다.

## 품목코드 / 요청번호

- 품목은 이름이 아니라 **품목코드**(`code`, 고유값)로 식별됩니다. 품목명은 향후 변경될 수 있어 검색·매칭·연동의 안정적인 기준이 되지 못하기 때문입니다. 일괄 업로드는 화면에서 파일 묶음마다 품목을 직접 검색해 선택하는 방식이며, 승인자·승인일·사유를 필수로 입력하면 완료 상태의 발주 건(요청번호 포함)이 자동으로 하나 생성됩니다.
- 모든 발주 건에는 등록 시점에 `RQ-YYYYMMDD-###` 형식의 **요청번호**가 자동 채번되어 대시보드·상세페이지·알림에 일관되게 표시됩니다.
